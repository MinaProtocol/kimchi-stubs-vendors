...
typedef enum class { YOUNG, OLD, UNTRACKED, MARKED_FOR_DEALLOCATION } class;
...
static pool **const global_rings[] = {
    &pools.old_full,      &pools.old_available,
    &pools.old_low,       &pools.young_available,
    &pools.young_full,    &pools.free,
    &pools.uninitialised, NULL};

static const class global_ring_classes[] = {OLD,   OLD,       OLD,      YOUNG,
                                            YOUNG, UNTRACKED, UNTRACKED};
...
*/
#define FOREACH_GLOBAL_RING(global_ring, cl, action)                           \
  do {                                                                         \
  ...
  FOREACH_GLOBAL_RING(global, cl, {
    ...
    do {
      if (p->hd.class == MARKED_FOR_DEALLOCATION) {
        if (global == &pools.free)
          --stats.live_pools;
      } else {
        if (last == NULL)
          start = p;
        else if (p->hd.prev != last)
          ring_link(last, p);
        last = p;
      }
      p = p->hd.next;
...
#define NUM_DEALLOC_THRESHOLD                                                  \
  (POOL_SIZE / (DEALLOC_THRESHOLD_SIZE * sizeof(slot)))
...
typedef enum occupancy { EMPTY, LOW, HIGH, QUASI_FULL, NO_CHANGE } occupancy;
...
static occupancy promotion_occupancy(pool *p) {
  ...
  if (threshold == 0)
    return EMPTY;
  if (threshold <= LOW_COUNT_THRESHOLD)
    return LOW;
  if (threshold <= HIGH_COUNT_THRESHOLD)
    return HIGH;
  return QUASI_FULL;
}
...
static occupancy demotion_occupancy(pool *p) {
  ...
  if (threshold == 0)
    return EMPTY;
  if (threshold == LOW_COUNT_THRESHOLD && p->hd.class == OLD)
    return LOW;
  if (threshold == HIGH_COUNT_THRESHOLD)
    return HIGH;
  return NO_CHANGE;
...
static slot *alloc_slot_slow(int for_young_block) {
  ...
  pool **available_pools =
      for_young_block ? &pools.young_available : &pools.old_available;
...
value boxroot_get(boxroot root) { return *(value *)root; }

value const *boxroot_get_ref(boxroot root) { return (value *)root; }
...
static int scan_pool(scanning_action action, pool *pool) {
  ...
  while (allocs_to_find) {
    ...
    if (LIKELY((!is_pool_member(s, pool)))) {
      --allocs_to_find;
      value v = (value)s;
      if (DEBUG && Is_block(v) && Is_young(v))
        ++stats.young_hit;
...
void boxroot_print_stats() {
  ...
  int ring_operations_per_pool =
      average(stats.ring_operations, stats.total_alloced_pools);

  if (!boxroot_used() && total_scanning_work == 0)
    return;
  ...
  printf("work per minor: %'d\n"
         ...
         scanning_work_minor, scanning_work_major, total_scanning_work,
         stats.total_scanning_work_minor, stats.total_scanning_work_major);
  ...
  printf("average time per minor: %'lldns\n"
         ...
         (long long)time_per_minor, (long long)time_per_major,
         (long long)stats.peak_minor_time, (long long)stats.peak_major_time);
#endif

  printf("total ring operations: %'d\n"
         "ring operations per pool: %'d\n",
         stats.ring_operations, ring_operations_per_pool);

#if DEBUG != 0
  printf("total created: %'d\n"
         "total deleted: %'d\n"
         "total modified: %'d\n",
         stats.total_create, stats.total_delete, stats.total_modify);
  ...
  printf("is_young_block: %'lld\n"
         ...
         stats.get_pool_header, stats.is_pool_member, stats.is_end_of_roots);
#endif
}
...
static void scanning_callback(scanning_action action) {
  ...
  if (boxroot_used()) {
    ...
    int64_t *total =
        in_minor_collection ? &stats.total_minor_time : &stats.total_major_time;
    int64_t *peak =
        in_minor_collection ? &stats.peak_minor_time : &stats.peak_major_time;
    *total += duration;
    if (duration > *peak)
      *peak = duration;
  }
...
static void record_minor_begin() {
  ...
  if (prev_minor_begin_hook != NULL)
    prev_minor_begin_hook();
}

static void record_minor_end() {
  in_minor_collection = 0;
  if (prev_minor_end_hook != NULL)
    prev_minor_end_hook();
}
